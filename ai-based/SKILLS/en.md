...
These are the core competencies any AI development team needs to address the OWASP Top 10 .

| **Skill** | **Description** | **OWASP Mapped Risks** |
| :--- | :--- | :--- |
| **AI Gateway & Guardrails** | Implement a runtime layer that acts as a security policy enforcement point. It must sanitize all inputs and outputs, redact sensitive data, and enforce budget limits. The skill is in *configuring* and *maintaining* this gateway, not just deploying it. | LLM01, LLM02, LLM07, LLM10 |
| **Adversarial Red Teaming** | The ability to systematically test the LLM application from an attacker's perspective. This includes techniques like multi-turn jailbreaks, prompt extraction, and system prompt probing. It's a distinct skill from standard pentesting . | LLM01, LLM05, LLM07, LLM09 |
| **Agent Security (Least Privilege)** | Designing and enforcing strict permissions for LLM agents. This means limiting tools, network access, and data scopes to the absolute minimum needed. The core skill is in *designing* and *implementing* a robust, policy-driven agent framework . | LLM06, LLM08 |
| **Data & Supply Chain Hygiene** | Validating the integrity of training data, models, and third-party dependencies (e.g., Python packages, LoRA adapters). Key skills are provenance tracking, integrity checking (hashing/signing), and secure sourcing of data . | LLM03, LLM04, LLM08 |
| **MLOps & Observability** | Integrating security checks into the machine learning lifecycle. This includes automating scans for poisoning, drift, and vulnerabilities, and having the observability to monitor for anomalous behavior and consumption spikes in production. | LLM03, LLM04, LLM08, LLM10, LLM09 |


# note

Agent must follow this checklist:
